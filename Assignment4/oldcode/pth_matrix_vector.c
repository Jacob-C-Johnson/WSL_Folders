...
int main(int argc, char *argv[]) {
    ...
    y = calloc(m, sizeof(double)); // Zero-initialized result vector
    ...
    Write_matrix_to_file(fileY, y, m, 1);
...
void* pth_mat_vect(void* rank) {
    ...
    for (int i = my_first_row; i <= my_last_row; i++) {
        for (int j = 0; j < p; j++) {  // Loop over columns of X
